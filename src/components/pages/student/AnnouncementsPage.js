            ...
            <div className="ap-main-layout">
                ...
                <main className="ap-main-content">
                    ...
                    <div className="ap-announcements-container">
                        ...
                        <div className="ap-sidebar-container">
                            <div className="ap-card">
                                ...
                                <div className="ap-card-body">
                                    {recentAnnouncements.length > 0 ? (
                                        recentAnnouncements.map((announcement) => (
                                            <div
                                                key={announcement.id}
                                                className="ap-recent-announcement"
                                                onClick={() => handleAnnouncementClick(announcement.id)}
                                            >
                                                <div className="ap-recent-title">
                                                    {announcement.isNew && <span className="ap-mini-new-badge"></span>}
                                                    {announcement.title}
                                                </div>
                                                <div className="ap-recent-meta">
                                                    <span className={`ap-recent-category ${getCategoryColorClass(announcement.course)}`}>
                                                        {announcement.course}
                                                    </span>
                                                    <span className="ap-recent-date">{announcement.date}</span>
                                                </div>
                                            </div>
                                        ))
                                    ) : (
                            ...
                            <div className="ap-card">
                                ...
                                <div className="ap-card-body">
                                    <div className="ap-category-list">
                                        <div
                                            className={`ap-category-item ${filter === 'all' ? 'active' : ''}`}
                                            onClick={() => setFilter('all')}
                                        >
                                            <span className="ap-category-name">전체</span>
                                            <span className="ap-category-count">{categoryCounts.all}</span>
                                        </div>
                                        <div
                                            className={`ap-category-item ${filter === 'admin' ? 'active' : ''}`}
                                            onClick={() => setFilter('admin')}
                                        >
                                            <span className="ap-category-name">학사/일반공지</span>
                                            <span className="ap-category-count">{categoryCounts.admin}</span>
                                        </div>
                                        <div
                                            className={`ap-category-item ${filter === 'course' ? 'active' : ''}`}
                                            onClick={() => setFilter('course')}
                                        >
                                            <span className="ap-category-name">강의공지</span>
                                            <span className="ap-category-count">{categoryCounts.course}</span>
                                        </div>

                                        {Object.keys(categoryCounts.courses).length > 0 && (
                                            <div className="ap-course-categories">
                                                {Object.entries(categoryCounts.courses).map(([courseName, count]) => (
                                                    <div
                                                        key={courseName}
                                                        className="ap-category-item ap-course-category"
                                                        onClick={() => {
                                                            setFilter('course');
                                                            setSearchTerm(courseName);
                                                        }}
                                                    >
                                                        <span className="ap-category-name">{courseName}</span>
                                                        <span className="ap-category-count">{count}</span>
                                                    </div>
                                                ))}
                                            </div>
                                        )}
